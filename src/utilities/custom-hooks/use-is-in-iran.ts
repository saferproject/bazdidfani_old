...
const useIsInIran = () => {
	...
	const { data: address } = useGetAddressQuery(currentLocation, { skip: !currentLocation });
	...
	useEffect(() => {
		if (import.meta.env.VITE_DETECT_LOCATION === "YES") {
			...
			const handleError = (err: GeolocationPositionError) => {
				switch (err.code) {
					case 1:
						setError("مرورگر دسترسی به موقعیت مکانی ندارد. از فعال بودن دسترسی موقعیت مکانی برای مرورگر اطمینان حاصل کنید.");
						break;

					case 2:
						setError("مرورگر نتوانست موقعیت را از جی پی اس دریافت کند. از روشن بودن جی پی اس اطمینان حاصل کنید.");
						break;

					case 3:
						setError("دریافت موقعیت مکانی بیش از حد طول کشید. در فضایی باز بروید و دوباره امتحان کنید.");
						break;
				}
