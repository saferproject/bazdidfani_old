...
import { getAccessibleCompanies } from "../utilities/get-active-company";
...
const DashboardLayout = () => {
  ...
  const changePasswordDialog = useAppSelector(
    (state) => state.changePasswordDialog,
  );
  ...
  return (
    <>
      {saferTextDialogData?.isOpen && (
        <SaferTextDialog {...saferTextDialogData} />
      )}
      {changePasswordDialog?.isOpen && <ChangePasswordDialog />}
      {(userData.isSuccess || userData.isUninitialized) && (
        <ResponsiveDrawer
          accessibleCompanies={getAccessibleCompanies(userData.data)}
          isCompanyContextFetching={userData.isFetching}
          refetchCompanyContext={userData.refetch}
        >
          <RemoveAuthStep />
