...
const LoginAsDialog: FC<LoginAsDialogProps> = ({
  isOpen,
  userId,
  fullName,
  onClose,
  ...
  const handleConfirm = async () => {
    ...
    try {
      const result = await (isLoginFromUser ? loginAsUserFn : loginAsFn)({
        userId,
      }).unwrap();
      ...
      dispatch(setUserID(String(result.data.user?.id ?? userId)));
      if (result.data.user?.username)
        dispatch(setPhone(result.data.user.username));
      ...
      window.location.href = isProfileIncompleteResponse(result)
        ? COMPLETE_PROFILE_PATH
        : "/dashboard";
    } catch {
    ...
    <Dialog open={isOpen} onClose={onClose} maxWidth="xs" fullWidth={true}>
      <div className="p-2 lg:p-4">
        ...
        <DialogContent className="flex flex-col items-center gap-4 text-center">
          <LoginCurve size="48" className="text-primary" />
          <p className="leading-7">
            ...
            وارد شوید؟ نشست فعلی شما حفظ شده و با خروج از حساب، دوباره به حساب
            مدیریت بازمی‌گردید.
          </p>
        </DialogContent>
