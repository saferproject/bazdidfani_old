...
const FleetList: FC<iprops> = ({ onSelect, isDialog, owner = "company" }) => {
	...
	const [customDialogProps, setCustomDialogProps] = useState<CustomDialogProps>({ ...EmptyCustomDialoProps });
	...
	useEffect(() => {
		if (owner === "company" && paginatorProps && filters && !isPhone) companyFleet.refetch();
	}, [paginatorProps, filters, isPhone]);

	useEffect(() => {
		if (owner === "company" && filters && isPhone) infiniteCompanyFleet.refetch();
	}, [filters, isPhone]);

	useEffect(() => {
		if (owner === "me" && paginatorProps && filters && !isPhone) driverFleet.refetch();
	}, [paginatorProps, filters, isPhone]);
	...
	const [changeFleetStatusFn, changeFleetStatusResult] = useChangeFleetStatusMutation();
		...
		(e: any, id: number) => {
			if (e.target.checked) {
				setCustomDialogProps({
					children: (
						<div className="flex flex-col items-center justify-between space-y-20 border-2 border-solid border-green-800 px-10 py-10">
							<p className="font-semibold text-lg">آیا از فعال کردن این ناوگان مطمئن هستید ؟</p>
				...
				setCustomDialogProps({
					children: (
						<div className="flex flex-col items-center justify-between space-y-20 border-2 border-solid border-green-800 px-10 py-10">
							<p className="font-semibold text-lg">آیا از غیر فعال کردن این ناوگان مطمئن هستید ؟</p>
							<div className="flex items-center justify-around w-full">
								<Button
									loading={changeFleetStatusResult.isLoading}
									variant="contained"
	...
	const columns: GridColDef[] = [
		...
		},
		{
			...
			renderCell: ({ row }) => (
				<div className="flex items-center justify-between">
					{changeFleetStatusResult.isLoading && changeFleetStatusResult.originalArgs.id === row.id ? (
		...
		},
		{
			...
			valueFormatter: (value) => (value === "freighter" ? "باری" : value === "passenger" ? "مسافری" : ""),
		},
		{
			field: "loaderType",
			...
			renderCell: ({ value, row }) =>
				value && (
					<p className="w-full flex justify-between px-4">
						<span className="text-gray-400 ml-2">{row.truck.usage === "freighter" ? "Kg" : "مسافر"}</span>
						<span>{value}</span>
					</p>
				),
		},
		...
		},
		{
			...
			renderCell: ({ value }) => (
				<div className="w-full flex justify-between px-2">
					<span>{value}</span>
					{value.toString().match(/^(19|20)/) ? <span className="text-gray-400">میلادی</span> : <span className="text-gray-400">شمسی</span>}
				</div>
			),
		},
		{
	...
	return (
		<div className="flex flex-col gap-8">
			...
			<SaferGrid<any>
				columns={columns}
				loading={driverFleet.isLoading || driverFleet.isFetching || companyFleet.isLoading || companyFleet.isFetching}
				rows={
					owner === "me"
						? isPhone
							? infiniteDriverFleet.data?.pages.map((page) => page.data.data).reduce((a, b) => [...a, ...b]) ?? []
							: driverFleet.data?.data.data ?? []
						: isPhone
						? infiniteCompanyFleet.data?.pages.map((page) => page.data.data).reduce((a, b) => [...a, ...b]) ?? []
				...
				paginatorProps={{
					...paginatorProps,
					totalPages: driverFleet.data?.data.last_page || companyFleet.data?.data.last_page,
				...
				fetchMoreData={owner === "company" ? infiniteCompanyFleet.fetchNextPage : infiniteDriverFleet.fetchNextPage}
				hasMore={owner === "company" ? infiniteCompanyFleet.hasNextPage : infiniteDriverFleet.hasNextPage}
			/>
		</div>
	);
};
