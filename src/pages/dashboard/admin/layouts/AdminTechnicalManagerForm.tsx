...
import TechnicalManagerFormSchema, {
  TechnicalManagerFormType,
} from "../../../RoleAssignment/schemas/technical-manager-form.schema";
import {
...
const AdminTechnicalManagerForm: FC<AdminTechnicalManagerFormProps> = ({
  ...
  const [freighterCertificationImage, setFreighterCertificationImage] =
    useState<CertificateImage | null>(null);
  const [passengerCertificationImage, setPassengerCertificationImage] =
    useState<CertificateImage | null>(null);
  ...
  const { capacity, freighter_capacity, passenger_capacity, type } =
    useWatch<TechnicalManagerFormType>({ control });

  const [addTechnicalManagerFn, addTechnicalManagerResult] =
    useAddAdminTechnicalManagerMutation();
  const [editTechnicalManagerFn, editTechnicalManagerResult] =
    useEditAdminTechnicalManagerMutation();

  const getCompanies = useGetCompaniesQuery({ status: 3 });
  ...
  const onSubmit = async (data: TechnicalManagerFormType) => {
    ...
    if (
      ...
      !freighterCertificationImage.image.includes(
        "https://api.bazdidfani.ir/storage/",
      ) &&
      !freighterCertificationImage.image.includes(
        "https://test-backend.bazdidfani.ir/storage/",
      )
    )
      newFormData.append(
        ...
        await compressImage(
          await dataUrlToFile(
            freighterCertificationImage.image,
            freighterCertificationImage.name,
          ),
          {
            maxSizeMB: 0.5,
            useWebWorker: true,
    ...
    if (
      ...
      !passengerCertificationImage.image.includes(
        "https://api.bazdidfani.ir/storage/",
      ) &&
      !passengerCertificationImage.image.includes(
        "https://test-backend.bazdidfani.ir/storage/",
      ...
      newFormData.append(
        ...
        await compressImage(
          await dataUrlToFile(
            passengerCertificationImage.image,
            passengerCertificationImage.name,
          ),
          {
            maxSizeMB: 0.5,
            useWebWorker: true,
  ...
  const setFormData = (data: TechnicalManagerRoleData) => {
    ...
    const imageFreighter = data.user.images?.find(
      (image) => image.image_type === "freighter",
    );
    const imagePassenger = data.user.images?.find(
      (image) => image.image_type === "passenger",
    );
  ...
  useEffect(() => {
    if (errors.national_code?.type === "pattern")
      setTimeout(() => clearErrors("national_code"), 3000);
  }, [errors.national_code]);

  useEffect(() => {
    if (errors.phone?.type === "pattern")
      setTimeout(() => clearErrors("phone"), 3000);
  }, [errors.phone]);

  useEffect(() => {
    if (errors.capacity?.type === "pattern")
      setTimeout(() => clearErrors("capacity"), 3000);
  }, [errors.capacity]);

  useEffect(() => {
    if (errors.freighter_capacity?.type === "pattern")
      setTimeout(() => clearErrors("freighter_capacity"), 3000);
  }, [errors.freighter_capacity]);

  useEffect(() => {
    if (errors.passenger_capacity?.type === "pattern")
      setTimeout(() => clearErrors("passenger_capacity"), 3000);
  ...
  return (
    ...
    >
      <Controller
        ...
        rules={{ required: true }}
        render={({ field }) => (
          <FormControl required className="lg:col-start-1 lg:col-end-6">
            ...
            <RadioGroup
              ...
              defaultValue={null}
            >
              <FormControlLabel value={1} label="باری" control={<Radio />} />
              <FormControlLabel value={2} label="مسافری" control={<Radio />} />
              <FormControlLabel
                value={3}
                label="باری و مسافری"
                control={<Radio />}
        ...
        onChange={(event) => {
          ...
          if (newValue.match(/\D/g))
            setError("national_code", {
              message: "کد ملی نمیتواند شامل حروف یا فاصله باشد",
              type: "pattern",
            });
      ...
      <TextField
        ...
        onChange={(event) => {
          ...
          if (newValue.match(/\D/g))
            setError("phone", {
              message: "شماره همراه نمیتواند شامل حروف یا فاصله باشد",
              type: "pattern",
            });
      ...
      <TextField
        ...
        onChange={(event) => {
          ...
          if (newValue.match(/\D/g))
            setError("capacity", {
              message: "حداکثر پذیرش روزانه نمیتواند شامل حروف یا فاصله باشد",
              type: "pattern",
            });
      ...
      {type !== 2 && (
        <TextField
          ...
          onChange={(event) => {
            ...
            if (newValue.match(/\D/g))
              setError("freighter_capacity", {
                message: "حداکثر پذیرش باری نمیتواند شامل حروف یا فاصله باشد",
                type: "pattern",
              });
      ...
      {type !== 1 && (
        <TextField
          ...
          onChange={(event) => {
            ...
            if (newValue.match(/\D/g))
              setError("passenger_capacity", {
                message: "حداکثر پذیرش مسافری نمیتواند شامل حروف یا فاصله باشد",
                type: "pattern",
              });
      ...
      <Box className="flex flex-row items-stretch gap-4! col-span-4 w-full! justify-start">
        ...
        <Button
          ...
          size="large"
          loading={
            addTechnicalManagerResult.isLoading ||
            editTechnicalManagerResult.isLoading
          }
        >
