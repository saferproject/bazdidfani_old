...
const AdminTechnicalManagersList: FC<AdminTechnicalManagersListProps> = ({ onEditTechnicalManager }) => {
	...
	const infiniteTechnicalManagers = useGetAdminInfiniteTechnicalManagersInfiniteQuery(
		{ page: paginatorProps.currentPage, per_page: paginatorProps.itemsPerPage, ...filters },
		{ skip: !paginatorProps || !filters || !isPhone }
	);

	const [changeTechnicalManagerStatusFn, changeTechnicalManagerStatusResult] = useChangeAdminTechnicalManagerStatusMutation();
	...
	}, [changeTechnicalManagerStatusResult.isSuccess, changeTechnicalManagerStatusResult.data]);
	...
	const columns: readonly GridColDef<any>[] = [
		...
		},
		{
			...
			renderCell: ({ row }) => (
				<div className="flex items-center justify-center w-full">
					{changeTechnicalManagerStatusResult.isLoading && changeTechnicalManagerStatusResult.originalArgs.userId == row.id ? (
			...
			renderCell: ({ row }) => (
				<div className="flex items-center justify-center">
					...
					<IconButton
						...
						onClick={() => setLoginAsTarget({ userId: row.user_id, fullName: row.user.personal.full_name })}
					>
						<Login
							size="24"
		...
		},
		{
			...
			editable: false,
			flex: 0.5,
			valueFormatter: (value) => (value === 1 ? "باری" : value === 2 ? "مسافری" : value === 3 ? "مسافری باری" : undefined),
		},
		{
			field: "username",
		...
		},
		{
			...
			align: "left",
			flex: 1,
			valueGetter: (_value, row) => row.companies.map((company) => company.name).join("، "),
		},
		{
			field: "capacity",
	...
	return (
		<div className="w-full flex flex-col gap-8">
			...
			<SaferGrid<any>
				...
				rows={
					isPhone
						? infiniteTechnicalManagers.data?.pages.map((page) => page.data.data).reduce((a, b) => [...a, ...b]) ?? []
