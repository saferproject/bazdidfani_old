...
export const {
	...
	endpoints: (builder) => ({
		getAdminTechnicalManagers: builder.query<APIResponse<Inspector>, Record<string, string | number | boolean> | null>({
			query: (params) => ({
				url: `admin/technical-manager/index${params ? "?" + buildQueryParams(params) : ""}`,
				method: "GET",
			}),
			providesTags: ["TechnicalManagers"],
			transformResponse: ({ data, message, status }: APIResponse<Inspector>) => ({
				data: {
					...data,
		...
		getAdminInfiniteTechnicalManagers: builder.infiniteQuery<APIResponse<Inspector>, Record<string, string | number | boolean>, number>({
		...
		changeAdminTechnicalManagerStatus: builder.mutation<any, { status: 0 | 1; userId: number; description: string }>({
