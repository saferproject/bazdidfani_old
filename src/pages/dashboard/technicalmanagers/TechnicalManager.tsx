...
export default function TechnicalManager() {
	...
	const handleCloseEditDialog = () => {
		setEditFormProps((currentValue) => ({ ...currentValue, data: null, isOpen: false }));
	};
	...
	const [editFormProps, setEditFormProps] = useState<EditTechnicalManagerDataDialogProps>({
		isOpen: false,
		data: null,
	...
	const [changeStatusUserFn, changeStatusUserResult] = useChangeTechnicalManagerStatusMutation();
		...
		(e: any, userId: number) => {
			if (e.target.checked) {
				setCustomDialogProps({
					children: (
						<div className="flex flex-col items-center justify-between space-y-20 border-2 border-solid border-green-800 px-10 py-10">
							<p className="font-semibold text-lg">آیا از فعال کردن این مدیر فنی مطمئن هستید ؟</p>
				...
				setCustomDialogProps({
					children: (
						<div className="flex flex-col items-center justify-between space-y-20 border-2 border-solid border-green-800 px-10 py-10">
							<p className="font-semibold text-lg">آیا از غیر فعال کردن این مدیر فنی مطمئن هستید ؟</p>
							<div className="flex items-center justify-around w-full">
								<Button
									loading={changeStatusUserResult.isLoading}
									variant="contained"
	...
	const handleEditInspector = (data) => {
		setEditFormProps((currentValues) => ({ ...currentValues, isOpen: true, data }));
	};
	...
	const columns: GridColDef<any>[] = [
		...
		},
		{
			...
			renderCell: ({ row }) => (
				<IconButton
					title="ویرایش"
					onClick={() => handleEditInspector(row)}
				>
					<Edit
						size="24"
	...
	return (
		<div className="flex flex-col gap-y-4 overflow-x-hidden">
			...
			{editFormProps.isOpen && <EditTechnicalManagerDataDialog {...editFormProps} />}
			...
			<SaferGrid
				...
				rows={
					isPhone
						? infiniteTechnicalManager.data?.pages.map((page) => page.data.data).reduce((a, b) => [...a, ...b]) ?? []
						: technicalManagers.data?.data.data.map((inspector) => inspector.personal) ?? []
				}
				loading={technicalManagers.isLoading || technicalManagers.isFetching}
				onCloseFilterDialog={() => {}}
				onFilterChange={() => {}}
