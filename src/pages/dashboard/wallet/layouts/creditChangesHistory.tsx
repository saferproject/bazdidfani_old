...
import { useGetCreditChangesHistoryQuery, useGetInfiniteCreditChangesHistoryInfiniteQuery } from "../api/wallet.api";
...
const CreditChangesHistory: FC<CreditChangeHistoryProps> = () => {
	...
	const infiniteCreditChangeHistory = useGetInfiniteCreditChangesHistoryInfiniteQuery(
		{ page: paginatorProps.currentPage, per_page: paginatorProps.itemsPerPage, ...filters },
		{ skip: !paginatorProps || !filters || !isPhone }
	);
	...
	const columns: readonly GridColDef<any>[] = [
		...
		},
		{
			...
			headerAlign: "center",
			editable: false,
			renderCell: ({ row }) => (row.transaction_type === "1" ? "شارژ" : "برداشت"),
		},
		{
			field: "price",
	...
	return (
		<>
			<Divider className="my-4">
				<h3 className="font-Yekan-Bakh font-semibold px-8 md:text-2xl">تاریخچه تراکنش ها</h3>
			</Divider>
			<SaferFilters
				mode="SEARCH_PARAMS"
				date={true}
			...
			<SaferGrid<any>
				columns={columns}
				loading={creditChangeHistory.isLoading || creditChangeHistory.isFetching}
				rows={
					isPhone
						? infiniteCreditChangeHistory.data?.pages.map((page) => page.data.data).reduce((a, b) => [...a, ...b]) ?? []
