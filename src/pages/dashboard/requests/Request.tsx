...
} from "../../../api/Requests/RequestsList";
...
export default function Request() {
	...
	const handleCloseDialog = () => setRedirectInspectionDialog((currentValue) => ({ ...currentValue, isOpen: false }));
	...
	const handleOpenInspectionRedirectDialog = (row) => {
		setRedirectInspectionDialog((currentValue) => ({ ...currentValue, isOpen: true, data: row }));
	};
	...
	const handleCloseMapDialog = () => setMapDialog((currentValue) => ({ ...currentValue, isOpen: false }));

	const handleShowInspectionLocation = (inspection: InspectionRequest) => {
		setMapDialog((currentValue) => ({
			...currentValue,
			isOpen: true,
			location: [Number(inspection.technical_inspection.latitude), Number(inspection.technical_inspection.longitude)],
			disableMouseEvents: true,
		}));
	};

	const handleCloseNewInspectionRequestDialog = () => setNewInspectionRequestDialog((currentValue) => ({ ...currentValue, isOpen: false }));

	const handleOpenCancelInspectionDialog = (inspectionRequest: InspectionRequest) => {
	...
	const handleOpenHazardousShipmentInspectionDialog = () =>
		setHazardousShipmentInspectionDialog((currentData) => ({ ...currentData, isOpen: true }));

	const handleCloseHazardousShipmentInspectionDialog = () =>
		setHazardousShipmentInspectionDialog((currentData) => ({ ...currentData, isOpen: false }));

	const handleGetExcel = async () => {
		const excelFile = await fetch(
			`${
				import.meta.env.VITE_IS_TEST_API === "YES" ? import.meta.env.VITE_TEST_API_URL : import.meta.env.VITE_API_URL
	...
	const [checkingInspection, setCheckingInspection] = useState<InspectionRequest | null>(null);
	const [cancelingInspection, setCancelingInspection] = useState<InspectionRequest | null>(null);
	const [paginatorProps, setPaginatorProps] = useState({ currentPage: 1, itemsPerPage: 10 });
	const [filters, setFilters] = useState(null);
	const [checkInspectionDialog, setCheckInspectionDialog] = useState<CheckInspectionDialogProps>({
		isOpen: false,
		data: null,
		onClose: handleCloseCheckInspectionDialog,
	});
	const [redirectInspectionDialog, setRedirectInspectionDialog] = useState<ReferInspectionDialogProps>({
		isOpen: false,
		data: null,
	...
	const [newInspectionRequestDialog, setNewInspectionRequestDialog] = useState<InspectionRequestFormProps>({
		isOpen: false,
	...
	const [hazardousShipmentInspectionDialog, setHazardousShipmentInspectionDialog] = useState<HazardousShipmentInspectionDialogProps>({
		isOpen: false,
		inspectionItems: [],
	...
	const infiniteRequest = useGetInfiniteRequestsListInfiniteQuery({ per_page: 10, ...filters }, { skip: !filters || !isPhone });
	const InspectionData = useGetInspectionCheckDataQuery({ id: checkingInspection?.id ?? 0 }, { skip: !checkingInspection });
	const { states, getStatus, getStatusColorClass } = useGetInspectionStates();

	const [cancelInspectionRequestFn, cancelInspectionRequestResult] = useCancelInspectionRequestMutation();
	...
	useEffect(() => {
		if (InspectionData.isSuccess)
			setCheckInspectionDialog((cuurentValue) => ({ ...cuurentValue, isOpen: true, data: InspectionData.data.data }));
	...
	}, [cancelInspectionRequestResult.isSuccess, cancelInspectionRequestResult.isError]);
	...
	const columns: readonly GridColDef<any>[] = [
		...
		},
		{
			...
			renderCell: ({ row }) => (
				<div className="flex flex-row gap-2">
					...
					{row.status !== 3 && row.status !== 5 && (
						<>
							<IconButton
								...
								loading={(InspectionData.isFetching || InspectionData.isLoading) && checkingInspection.id === row.id}
								disabled={(InspectionData.isFetching || InspectionData.isLoading) && checkingInspection.id !== row.id}
							>
								<ReceiptSearch
									size="24"
					...
					{row.status !== 3 && row.status !== 11 && row.status !== 12 && row.status !== 14 && row.status !== 15 && row.status !== 16 && (
						<IconButton
							title="ابطال بازدید"
					...
					{row.technical_inspection.latitude && row.technical_inspection.longitude && (
						<IconButton
							title="موقعیت بازدید"
					...
					<PrintMenu loaderType={row.truck.loader_code} inspectionRequest={row} />
				</div>
			),
		},
		{
		...
		},
		{
			...
			renderCell: ({
				row: {
					truck: { first_number, second_number, third_character, fourth_number },
				},
			}) => (
				<Plate
		...
		},
		{
			...
			headerAlign: "center",
			editable: false,
			valueGetter: (_value, row) => GetShamsiDateTime(row.technical_inspection.start_technical_inspection_at),
		},
		{
			field: "user_company",
			headerName: "اپراتور",
			...
			headerAlign: "center",
			editable: false,
			valueGetter: (_, row) => row?.user_company?.full_name ?? row?.company_credential.name,
		},
		{
			field: "inspector",
			headerName: "مدیر فنی",
		...
		},
		{
			...
			editable: false,
			renderCell: ({ row }) => {
				return <p className="font-semibold">{getStatus(row.status)?.technical_inspection_title}</p>;
			},
		},
		{
			field: "driver_name",
		...
		},
		{
			...
			headerAlign: "center",
			editable: false,
			valueFormatter: (value) => (value === 2 ? "مسافری" : value === 1 ? "باری" : "نا مشخص"),
		},
		{
			field: "code",
			headerName: "رهگیری",
	...
	const handleAddRequest = (copiedData?: InspectionRequest) => {
		setNewInspectionRequestDialog((currentValue) => ({ ...currentValue, isOpen: true, data: copiedData }));
	};

	return (
		<div className="max-w-screen lg:max-w-[80vw] xl:max-w-[90vw] flex flex-col gap-4">
			{checkInspectionDialog.isOpen && <CheckInspectionDialog {...checkInspectionDialog} />}
			{redirectInspectionDialog.isOpen && <ReferInspectionDialog {...redirectInspectionDialog} />}
			{mapDialog.isOpen && <MapDialog {...mapDialog} />}
			{newInspectionRequestDialog.isOpen && <InspectionRequestForm {...newInspectionRequestDialog} />}
			{textDialog.isOpen && <SaferTextDialog {...textDialog} />}
			{hazardousShipmentInspectionDialog.isOpen && <HazardousShipmentInspectionDialog {...hazardousShipmentInspectionDialog} />}
			<Fab
				size="medium"
				color="primary"
			...
			<SaferGrid<any>
				...
				rows={
					isPhone
						? infiniteRequest.data?.pages.map((page) => page.data.data).reduce((a, b) => [...a, ...b]) ?? []
