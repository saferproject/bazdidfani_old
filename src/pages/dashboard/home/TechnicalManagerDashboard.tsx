...
const kpiIcons: Record<string, ReactNode> = {
  ...
  active_companies: <TbUsersGroup />,
  approved_percentage_month: <TbThumbUp />,
};
...
/** داشبورد مدیر فنی — داده‌ی واقعی از api/technical-manager/dashboard. */
const TechnicalManagerDashboard = () => {
  ...
  const {
    ...
    recentInspections,
    topCompanies,
  } = mapTechnicalManagerDashboard(data.data);
  ...
  return (
    <div className="flex flex-col gap-5 pb-6">
      <DashboardHeader
        title="داشبورد مدیر فنی"
        subtitle="وضعیت بازدیدها و چک‌لیست‌های شما در یک نگاه."
        items={today.map((t, i) => ({
          label: t.label,
      ...
      {/* آخرین بازدیدها + شرکت‌های تحت پوشش */}
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <SectionCard title="آخرین بازدیدهای من" className="lg:col-span-2">
        ...
        <SectionCard
          title="شرکت‌های تحت پوشش"
          action={<TbCircleCheck className="text-lg text-[#00be77]" />}
        >
            ...
            <div className="flex flex-col gap-4">
              {topCompanies.map((c) => (
                <ProgressRow
                  key={c.name}
                  label={c.name}
                  value={`${formatNumber(c.count)} بازدید`}
                  ratio={(c.count / maxCompany) * 100}
                  color="#7474C1"
                />
              ))}
            </div>
